...
*/
const externalPackageNameSchema = z.string().min(1).refine((value) => !value.startsWith(".") && !value.startsWith("/"), { error: "must be a package name such as \"microsandbox\" or \"@scope/pkg\", not a relative or absolute path" });
/**
...
*/
const stageablePackageName = (value) => {
...
const isNpmPackageName = (root) => {
	...
	if (rest.length > 0 || name === void 0) return false;
...
*/
const externalPackageEntrySchema = z.union([externalPackageNameSchema, z.strictObject({
...
const bundlerSchema = z.custom((value) => value === "esbuild" || value === "none" || typeof value === "function", { message: "bundler must be \"esbuild\", \"none\", or a function (fn) => Promise<FunctionBundle>" });
const isEsbuildBundler = (bundler) => bundler === void 0 || bundler === "esbuild";
...
const entryIncludesFiles = (entry) => typeof entry === "string" ? true : entry.includeFiles !== false;
/**
...
const functionDefSchema = z.strictObject({
	...
	if (ctx.value.externalPackages !== void 0 && !isEsbuildBundler(ctx.value.bundler)) {
		ctx.issues.push({
			code: "custom",
			input: ctx.value.externalPackages,
			path: ["externalPackages"],
			message: `externalPackages only applies to the "esbuild" bundler; the "${typeof ctx.value.bundler === "function" ? "custom" : ctx.value.bundler}" bundler controls its own output. Remove externalPackages or switch to the esbuild bundler`
		});
		return;
	}
	const seenNames = /* @__PURE__ */ new Map();
	const rootIntent = /* @__PURE__ */ new Map();
	entries.forEach((entry, index) => {
		...
		if (includeFiles && !stageablePackageName(name)) {
			ctx.issues.push({
				...
				message: `"${name}" does not name a single installable package, so its files cannot be staged. Name one package, or set includeFiles: false to leave the import unresolved without shipping anything for it`
			});
			return;
		}
		...
		if (prior !== void 0 && prior.includeFiles !== includeFiles) {
			ctx.issues.push({
				code: "custom",
				input: entry,
				path: ["externalPackages", index],
				message: `"${name}" and "${prior.at}" are both part of the "${root}" package but disagree about includeFiles; files ship per package, so the whole package either ships or does not`
			});
			return;
		}
		rootIntent.set(root, {
